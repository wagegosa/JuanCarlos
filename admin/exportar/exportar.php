...
if ($fecha_inicial != "" || $fecha_inicial != null){
	...
	$query = "SELECT fec_reunio_inicio, direccion, organizador, telefono, barrio FROM juancarlos.tba_reunion WHERE CONVERT(fec_reunio_inicio, DATE)  = '$fecha_inicial'";
	...
	->setCellValue('B3', $rows['fec_reunio_inicio'])
	->setCellValue('C3', 'Dirección:')
	->setCellValue('D3', $rows['direccion'])
	->setCellValue('A4', 'Organiza:')
	->setCellValue('B4', $rows['organizador'])
	->setCellValue('C4', 'Barrio:')
	->setCellValue('D4', $rows['barrio'])
	->setCellValue('E4', 'Teléfono')
	->setCellValue('F4', $rows['telefono'])
	->setCellValue('A6', 'Nro')
    ...
    $query = "SELECT idtba_regist_asiste, cedula, nom_comple, telefono, correo, dir_reside, Lug_votaci, usuario, organizador, fec_creaci FROM juancarlos.vs_exp_fecha WHERE CONVERT(fec_creaci, DATE)  = '$fecha_inicial'";
	// echo "<pre>";
	// print_r($queryR);
	// echo "</pre>";
	// die();
