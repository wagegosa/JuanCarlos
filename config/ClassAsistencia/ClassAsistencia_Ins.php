...
$usuario = $_P7OST['usuario'];
//Validamos que el metodo POST este enviando datos.
if ($_POST != "" ){
  try{
    $Con= new DataBase();
    $Conexion= $Con->Conexion();
    $query = "INSERT INTO Juancarloso.tba_regist_asiste 
                         (cedula, nom_comple, telefono, correo, dir_reside, lugar_votacio, usuario, activo, fec_creaci, fec_modifi, organizador) 
                  VALUES ('$num_cedula', '$nombre', '$telefono', '$email', '$direccion', '$lugar_votacio', '$usuario', 'Y', '$hoy', '$hoy', '$organizador')";
